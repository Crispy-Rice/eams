...
| DB_PASSWORD | 123456 | 123456（与 MySQL 容器一致） |
...
| `-e MYSQL_ROOT_PASSWORD=123456` | MySQL root 密码 |
...
# 验证表已创建
docker exec -it mysql mysql -uroot -p123456 school_db -e "SHOW TABLES;"
```
...
docker run -d --name eams-web --network eams-net -p 8000:8000 \
  -e DB_HOST=mysql -e DB_PORT=3306 -e DB_USER=root -e DB_PASSWORD=123456 -e DB_NAME=school_db \
  eams-web:1.0
```
