...
def add_class(data: ClassCreate):
    ...
    new_id = ClassModel().create(data.name, data.grade, data.head_teacher_id, data.if_youxiu, data.student_num, data.graduation_year)
...
def update_class(class_id: int, data: ClassUpdate):
    ...
    if ClassModel().get_by_id(class_id) is None:
        raise HTTPException(status_code=404, detail="班级不存在")
    ClassModel().update(class_id, data.name, data.grade, data.head_teacher_id, data.if_youxiu, data.student_num, data.graduation_year)
